...
namespace filter
{
    ...
    void fft(std::complex<double> dst[], const std::complex<double> src[], int n)
    {
        ...
        std::vector<std::complex<double>> x0(&src[0], &src[size]);
        std::vector<std::complex<double>> x1(size);
        std::complex<double> A = exp(std::complex<double>(0, -2 * M_PI / size));
        ...
        for(int r = 1; r <= n; ++r)
        {
            ...
            for(int i = 0; i < size; ++i)
            {
                ...
                s = reversal[s];
                s <<= n_r;                      // s *= bit;
                const std::complex<double>& src1 = x0[i & ~bit];
                const std::complex<double>& src2 = x0[i | bit];
                if(i & bit)
                {
