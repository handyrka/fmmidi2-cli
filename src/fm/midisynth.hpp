...
namespace midisynth
{
    ...
    {
    public:
        note(int ass, int pan) : assign(ass), panpot(pan){}
        virtual ~note(){}
    ...
    public:
        ...
        channel* get_channel(int ch) const;
    ...
    //FM drum program parameter
    struct drum_parameter : fm_parameter
    {
    ...
    {
    public:
        fm_sound_generator(const fm_parameter& params, int note, float frequency_multiplier);
    ...
    {
    public:
        fm_note(const fm_parameter& params, int note, int velocity, int panpot, int assign, float frequency_multiplier);
    ...
    public:
        ...
        void clear();
        bool set_program(int number, const fm_parameter& p);
        bool set_drum_program(int number, const drum_parameter& p);
        virtual note* note_on(int_least32_t program, int note, int velocity, float frequency_multiplier);

        void get_program(int number, fm_parameter& p);

    private:
        std::map<int, fm_parameter> programs;
        std::map<int, drum_parameter> drums;
    };
}