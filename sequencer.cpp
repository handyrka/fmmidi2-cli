...
namespace midisequencer
{
    ...
    sequencer::sequencer(): position(messages.begin())
    {
    }
    ...
    bool sequencer::load(void* fp, int(*fgetc)(void*))
    {
        ...
        else throw load_error("unsupported format");
    ...
    {
        if(fgetc(fp) != 0 || fgetc(fp) != 0 || fgetc(fp) != 0 || fgetc(fp) != 6 || fgetc(fp) != 0)
        {
        ...
        for(unsigned track = 0; track < num_tracks; ++track)
        {
            ...
            uint_least32_t track_length = (static_cast<uint_least32_t>(l0) << 24) | (static_cast<uint_least32_t>(l1) << 16) | (l2 << 8) | l3;
        ...
        if(!(division & 0x8000))
        {
            ...
            for(std::vector<midi_message>::iterator i = messages.begin(); i != messages.end(); ++i)
            {
                ...
                if((i->message & 0xFF) == 0xFF)
                {
                    ...
                    {
                        tempo = (static_cast<uint_least32_t>(static_cast<unsigned char>(s[1])) << 16) | (static_cast<unsigned char>(s[2]) << 8) | static_cast<unsigned char>(s[3]);
                        base = org_time;
                        time_offset = i->time;
                    }
                }
